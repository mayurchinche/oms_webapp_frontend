...
const ManageMaterialModal = ({ isModalOpen, closeModal }) => {
  ...
  const filteredMaterials = materials.filter((material) =>
    material.material_name && material.material_name.toLowerCase().includes(searchTerm.toLowerCase())
  );
              ...
              <Grid container spacing={2} sx={ManagingMaterialsStyles.materialsGrid}>
                {filteredMaterials.map((material, index) => (
                  <Grid item xs={12} sm={6} md={4} key={index}>
                    <Paper elevation={3} sx={ManagingMaterialsStyles.materialBox} ref={(el) => materialRefs.current[material.material_name] = el}>
                      <Typography sx={ManagingMaterialsStyles.subtitle}>{material.material_name}</Typography>
                      <Button
                        variant="outlined"
                        color="primary"
